...
- The mutation scripts are now drep's five, copied with this repository's role. Hosted and local runs share the role's host lock, a mirrored result is proven to come from its own run, a second run in one checkout waits for the first instead of deleting its results and scratch, and each checkout mirrors into its own directory on ai-1. A commit with no staged Rust changes exits before the lock. The tests are drep's contract and lock tests plus the existing verdict and hook tests, split into `mutation_scripts_test.rs` and `mutation_locks_test.rs`, and test stubs are written from a child process so Linux cannot refuse to exec one with `ETXTBSY`.
